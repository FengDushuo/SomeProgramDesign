...
class EnveController extends InterceptController {
 ...
 /*   	$adv = AdvController::instance()->getAdv('hbsquare');exit(json_encode($adv));exit();
    ...
    public function saveEnveReceive(){
        ...
        $home ="/www/wwwroot/www.010xr.com";
...
//             $post_data['voice_url'] = trim($post_data['voice_url'],'&quot;');
             $post_data['voice_url'] = trim($post_data['voice_url'],'/');
          		//$post_data['voice_url'] = '/data/upload/default/20180116/16k.pcm';
            //获取识别结果
            ...
            if(!$samplepy){
               //删除没有说话后的文件
                   ...
                   file_put_contents("/www/wwwroot/www.010xr.com/application/Api/Controller/删除pcm记录.txt",serialize($pcm_file));
                   file_put_contents("/www/wwwroot/www.010xr.com/application/Api/Controller/删除MP3记录.txt",serialize($mp3_file));
                   $cmd = "rm -f  $pcm_file" ; 
                   $res = shell_exec($cmd);
                   $cmd = "rm -f  $mp3_file" ; 
                   $res = shell_exec($cmd);
                ...
                $this->ajaxReturn(['code'=>40000, 'msg'=>'您没有说话哟',$resText=>$sampleText,$samplepy=>$aa,'11'=>$aa]);
            ...
            if( $samplepy != $compareStr){
                ...
                if($is_pass == false ){
                   ...
                   $res = shell_exec($cmd);
                    ...
                    $this->ajaxReturn(['code'=>40000, 'msg'=>$sampleText,$aa=>$samplepy,$str=>$info['quest_py']]);
